    ...
    internal class HostListenerFactory : IListenerFactory
    {
        ...
        public async Task<IListener> CreateAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (IFunctionDefinition functionDefinition in _functionDefinitions)
            {
                ...
                // For each listener that is a scale monitor, register it with the provider. Based on the above diabled
                // check, this means we'll only be monitoring valid, non-disabled functions which is what we want.
